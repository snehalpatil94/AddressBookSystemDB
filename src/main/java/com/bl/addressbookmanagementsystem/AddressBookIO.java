...
public class AddressBookIO {
    ...
    static final String FILE_PATH = "AddressBookDiectory1.txt";
    static final String CSV_FILE_PATH = "AddressBookDirctory2.csv";
    private static final String JSON_FILE_PATH = "AddressBookDirctory3.json";
    Contacts person = new Contacts();
    List<Contacts> contactList = new ArrayList<>();
    HashMap<String, AddressBook> contactService = new HashMap<>();
    ...
    public void addContact() {
        ...
        AddressBook addressBook = contactService.get(newContact);
        if (addressBook != null) {
    ...
    public void editContactInBook() {
        ...
        if (contactService.containsKey(editBookName)) {
            ...
            System.out.println("com.bl.addressBook.AddressBook doesn't exist!!");
            editContactInBook();
        }
    }
    ...
    public void searchByState(String state) {
        for (Map.Entry<String, AddressBook> entry : contactService.entrySet()) {
            List<Contacts> contactListByState = entry.getValue().contactList;
            List<Contacts> collect = contactListByState.stream()
                    .filter(p -> p.getState().equalsIgnoreCase(state)).collect(Collectors.toList());
            for (Contacts contactsss : collect) {
    ...
    public void searchByCity(String city) {
        for (Map.Entry<String, AddressBook> entry : contactService.entrySet()) {
            List<Contacts> contactListByCity = entry.getValue().contactList;
            List<Contacts> collect = contactListByCity.stream().filter(p -> p.getCity().equalsIgnoreCase(city))
    ...
    public void countByCity(String cityName) {
        for (Map.Entry<String, AddressBook> entry : contactService.entrySet()) {
            ...
            long countContactsByCity = contactListByCity.stream().filter(g -> g.getCity().equalsIgnoreCase(cityName))
                    .count();
            System.out.println("Total Number of com.bl.addressBook.Contact from '" + cityName + "' city is " + countContactsByCity);
        }
    }
    ...
    public void countByState(String stateName) {
        for (Map.Entry<String, AddressBook> entry : contactService.entrySet()) {
            ...
            System.out.println("Total Number of com.bl.addressBook.Contact from '" + stateName + "' state is" + countContactsByState);
        }
    }
    ...
    public void sortByName() {
        for (Map.Entry<String, AddressBook> entry : contactService.entrySet()) {
            System.out.println("The contacts in the Book of < " + entry.getKey() + " > are!...");
            List<Contacts> contactListByState = entry.getValue().contactList;
            List<Contacts> list = contactListByState.stream().collect(Collectors.toList());
    ...
    public void writeToFile() throws IOException {
        ...
        for (Map.Entry<String, AddressBook> entry : contactService.entrySet()) {
            ...
            List<Contacts> contList = entry.getValue().contactList;
